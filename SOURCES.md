...
All 20 skills, grouped by maintenance path. The 9 entries in `skills-lock.json` are tracked by the [`skills`](https://github.com/vercel-labs/skills) CLI and refreshed via `npx skills update`. 6 more sync from public upstreams the CLI cannot enumerate. The remaining 5 are Mosoo-maintained originals or adaptations.
...
- [`cloudflare`](./skills/cloudflare) — adapted from [`cloudflare/skills`](https://github.com/cloudflare/skills/tree/main/skills/cloudflare) so Sandbox guidance preserves an existing Wrangler format and derives a matching package/image version from repository and current-doc evidence
